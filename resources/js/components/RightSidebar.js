...
const RightSidebar = (props) => {
    ...
    return (
      <div className="w-3/12 flex flex-col bg-blue-gray-50 h-full bg-white pr-4 pl-2 pb-5 py-4">
        <div className="bg-white flex flex-col h-full shadow">
          ...
          <div className="flex-1 flex flex-col overflow-auto">
            <div className="h-16 text-center flex justify-center">
              <div className="pl-8 text-left text-lg py-4 relative">
                ...
                <div  className="text-center absolute bg-cyan-500 text-white w-5 h-5 text-xs p-0 leading-5  -right-2 top-3">
            ...
            <div className="flex-1 w-full px-4 overflow-auto">
              { cartItems.map(item => (
                <div key={item.id} className="select-none mb-3 bg-blue-gray-50  w-full text-blue-gray-700 py-2 px-2 flex justify-center">
                  <img src={item.image} alt="" className="h-10 w-10 bg-white shadow mr-2" />
                  ...
                  <div className="py-1">
                    <div className="w-28 grid grid-cols-3 gap-2 ml-2">
                      <button onClick={() => removeFromCart(item)} className="text-center py-1 text-white bg-blue-gray-600 hover:bg-blue-gray-700 focus:outline-none">
                      ...
                      <input readOnly onChange={() => item.qty} value={item.qty} type="text" className="bg-white  text-center shadow focus:outline-none focus:shadow-lg text-sm" />
                      <button onClick={() => addToCart(item)} className=" text-center py-1 text-white bg-blue-gray-600 hover:bg-blue-gray-700 focus:outline-none">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-3 inline-block" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div className="select-none h-auto w-full text-center pt-3 pb-4 px-4">
            ...
            <div className="mb-3 text-blue-gray-700 px-3 pt-2 pb-3 bg-blue-gray-50">
              <div className="flex text-lg font-semibold">
                <div className="flex-grow text-left">CASH</div>
                <div className="flex text-right">
                  <div className="mr-2">Rp</div>
                  <input readOnly value={numberFormat(cash)} type="text" className="w-28 text-right bg-white shadow focus:bg-white focus:shadow-lg px-2 focus:outline-none" />
                </div>
              </div>
              <hr className="my-2" />
              <div className="grid grid-cols-3 gap-2 mt-2">
                {moneys.map((money) => (
                  <button onClick={() => addCash(money)} key={money} className="bg-white shadow hover:shadow-lg focus:outline-none inline-block px-2 py-1 text-sm">+<span> { numberFormat(money) } </span></button>
                ))}
              </div>
            </div>
            <div className={"flex mb-3 text-lg font-semibold text-blue-gray-700 py-2 px-3 "+( change >= 0 ? "bg-cyan-50" : "bg-pink-100")}>
            ...
            <button onClick={() => submit()} disabled={change < 0 || cartItems.length <= 0} className={"text-white text-lg w-full py-3 focus:outline-none "+(change >= 0 && cartItems.length > 0 ? "bg-cyan-500 hover:bg-cyan-600" : "bg-blue-gray-200")}>
              SUBMIT
            </button>
          </div>
        </div>
